  ...
  if (existingSegmentedBy && existingSegmentedBy !== "none") {
    ...
    results.forEach((item) => {
      ...
      if (!groupedData[xValue]) {
        ...
        groupedData[xValue] = {
          label: xValue,
          segments: {}, // Change to object to merge segments
          id: id,
        };
      }
...
async function generatePersonPerformanceData(
  ...
  if (shouldGroupByDuration) {
    ...
    // For organization, exclude where LeadOrganization.organization is null
    xaxisNullExcludeCondition["$LeadOrganization.organization$"] = {
      [Op.ne]: null,
      [Op.ne]: "",
    };
...
function isDateField(xaxis) {
  const dateFields = ["createdAt", "dueDate"];
  return dateFields.includes(xaxis);
}

function getConditionObject(column, operator, value, includeModels = [], MasterUser, Lead, Deal, LeadPerson, LeadOrganization, Activity, Email) {
  ...
  if (isDateRangeFilter && Array.isArray(value)) {
    ...
    if (tableAlias !== "LeadPerson") {
      ...
      addIncludeModel(tableAlias, includeModels, LeadOrganization);
...
function addIncludeModel(tableAlias, includeModels, LeadOrganization) {
  let modelConfig;
...
async function generateExistingPersonPerformanceDataForSave(
  ...
  if (existingSegmentedBy && existingSegmentedBy !== "none") {
    ...
    results.forEach((item) => {
      ...
      if (existingxaxis === "organization" && item.leadOrganizationId) {
...
async function generatePersonPerformanceDataForSave(
  ...
  if (segmentedBy && segmentedBy !== "none") {
    ...
    results.forEach((item) => {
      ...
      if (xaxis === "organization" && item.leadOrganizationId) {
        // Group by leadOrganizationId for organization
        groupKey = `org_${item.leadOrganizationId}`;
      } else if (
...
exports.getPersonReportSummary = async (req, res) => {
  ...
  try {
    ...
    // Get total count
    const totalCount = await LeadPerson.count({
      where: baseWhere,
...
// // Helper function to generate activity performance data without pagination
// exports.generatePersonPerformanceDataForSave = async (
//   ownerId,
//   role,
//   xaxis,
//   yaxis,
