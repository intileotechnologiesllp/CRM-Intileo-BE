        ...
        async (msg) => {
          if (msg !== null) {
            ...
            let {
              ...
              skipCount,
              debugMode,
              // 📧 NEW: Enhanced folder configuration
              foldersToProcess,
              enforceSentFolder,
            } = JSON.parse(msg.content.toString());

            console.log(`[UserWorker] 📧 PROCESSING USER ${masterUserID}: Folders=[${foldersToProcess ? foldersToProcess.join(', ') : 'default'}], SentForced=${enforceSentFolder ? 'YES' : 'NO'}`);
                ...
                const fetchPromise = fetchInboxEmails(
                  {
                    ...
                    query: {
                      ...
                      skipCount,
                      debugMode,
                      // 📧 NEW: Enhanced folder configuration (also in query for fallback)
                      foldersToProcess: foldersToProcess ? foldersToProcess.join(',') : undefined,
                      enforceSentFolder,
                    },
                  },
                  {
                    status: (code) => ({
                      json: (data) => {
                        // 📧 Enhanced logging for sent folder processing
                        const sentResult = data.folderResults?.sent;
                        if (sentResult) {
                          console.log(`[UserWorker] 📧 SENT FOLDER RESULT for user ${masterUserID}:`);
                          console.log(`[UserWorker] 📧   - Processed: ${sentResult.processedCount} emails`);
                          console.log(`[UserWorker] 📧   - Status: ${sentResult.status}`);
                          console.log(`[UserWorker] 📧   - Folder: ${sentResult.folderName || 'unknown'}`);
                          if (sentResult.message) {
                            console.log(`[UserWorker] 📧   - Message: ${sentResult.message}`);
                          }
                        }
